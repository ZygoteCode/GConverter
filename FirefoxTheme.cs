    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        for (int i = 0; i <= TabPages.Count - 1; i++)
        {
            ...
            if (SelectedIndex == i)
            {
                ...
                using (SolidBrush B = new SolidBrush(Color.FromArgb(111, 84, 225)))
                {
            ...
            using (SolidBrush B = new SolidBrush(FC))
            {
                G.DrawString(TabPages[i].Text, Theme.GlobalFont(FontStyle.Regular, 10), B, new Point(TabRect.X + 20, TabRect.Y + 12));
            }
        }
